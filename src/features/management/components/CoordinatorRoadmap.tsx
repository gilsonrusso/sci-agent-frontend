...
export default function CoordinatorRoadmap() {
  ...
  const months = [
    ...
    {
      name: format(addDays(endOfMonth(startDate), 1), 'MMMM yyyy'),
      days: eachDayOfInterval({
        start: startOfMonth(addDays(endOfMonth(startDate), 1)),
        end: endOfMonth(addDays(endOfMonth(startDate), 1)),
      }).length,
    },
  ];
    ...
    >
        ...
        >
          <Box sx={{ display: 'flex', borderBottom: `1px solid ${theme.palette.divider}` }}>
            {months.map((m, i) => (
              ...
              >
                <Typography sx={{ color: 'text.secondary', fontSize: 12 }}>{m.name}</Typography>
              </Box>
            ))}
          </Box>
        </Box>
      ...
      <Box sx={{ flexGrow: 1, overflowY: 'auto' }}>
        {articles.map((article) => {
          ...
          const getRiskColor = () => {
            if (riskLevel === 'HIGH') return theme.palette.error.main;
            if (riskLevel === 'MEDIUM') return theme.palette.warning.main;
          ...
          const getRiskIcon = () => {
            ...
            if (riskLevel === 'MEDIUM')
              return <WarningAmber sx={{ fontSize: 14, color: 'warning.main' }} />;
          ...
          return (
            ...
            >
              ...
              >
                <Typography noWrap sx={{ color: 'text.primary', fontSize: 14, flexGrow: 1 }}>
                  {article.title}
                </Typography>
                {article.authors[0] && (
                  <Avatar
              ...
              >
                ...
                {offsetDays > -30 && ( // Just to show something on screen
                  <Tooltip
                    title={`Risk Level: ${riskLevel} - AI predicts delivery in ${durationDays} days`}
                  >
                    <Box
                      sx={{
                        position: 'absolute',
                    ...
                    >
                      ...
                      <Typography
                        noWrap
                        sx={{ color: getRiskColor(), fontSize: 11, fontWeight: 600 }}
                      >
